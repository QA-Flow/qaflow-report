...
class QAFlowReporter {
  ...
  async step<T>(
    ...
    try {
      ...
      this.currentTest.steps.push(step);
      this.currentTest.errors.push(error instanceof Error ? error : new Error(String(error)));
      return { step };
    }
  }
  ...
  async end(): Promise<{
    ...
    try {
      ...
      console.log(chalk.bold(
        `\n===== TEST RESULTS =====\n` +
        `Test: ${this.currentTest.session.name}\n` +
        `Duration: ${duration}ms\n` +
        `Status: ${this.getStatusWithColor(totalStatus)}\n`
      ));
      
      console.log(chalk.bold('Test Steps:'));
      
      console.log(
        this.currentTest.steps.map((step, index) => {
          const statusText = this.getStatusWithColor(step.status);
          return `${index + 1}. ${step.name}: ${statusText} ${step.duration ? `(${step.duration}ms)` : ''}`;
        }).join('\n')
      );
      ...
      if (this.currentTest.errors.length > 0) {
        console.log(chalk.bold('\nErrors:'));
        console.log(
          this.currentTest.errors.map((error, index) => 
            chalk.red(`${index + 1}. ${errorToString(error)}`)
          ).join('\n')
        );
      }
  ...
  /**
   * Get colored status text for console output
   * @param status Test status
   * @returns Colored status text
   */
  private getStatusWithColor(status: TestStatus): string {
    switch (status) {
